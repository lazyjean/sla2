...
func (s *CourseService) Update(ctx context.Context, req *pb.CourseServiceUpdateRequest) (*pb.CourseServiceUpdateResponse, error) {
	course, err := s.courseService.UpdateCourse(
		...
		req.Desc,
		req.CoverUrl,
		s.converter.ToEntityLevel(req.Level),
		s.converter.ToEntityCategory(req.Category),
		req.Tags,
		s.converter.ToEntityStatus(req.Status),
		req.Prompt,
		req.Resources,
		req.RecommendedAge,
		req.StudyPlan,
...
func (s *CourseService) Delete(ctx context.Context, req *pb.CourseServiceDeleteRequest) (*pb.CourseServiceDeleteResponse, error) {
	err := s.courseService.DeleteCourse(ctx, uint(req.Id))
	if err != nil {
		return nil, err
	}
...
func (s *CourseService) UpdateSection(ctx context.Context, req *pb.CourseServiceUpdateSectionRequest) (*pb.CourseServiceUpdateSectionResponse, error) {
	section, err := s.courseService.UpdateSection(ctx, entity.CourseSectionID(req.Id), req.Title, req.Desc, req.OrderIndex, s.converter.ToEntitySectionStatus(req.Status))
	if err != nil {
		return nil, err
	}
